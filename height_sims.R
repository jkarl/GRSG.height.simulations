# Simulation of different height methods for Sage-grouse habitat assessments
...
# Questions answered with the simulation:
...
## For the DiStefano et al. (2018) paper, the following simulations were run:
##   aim-haf_compare_d100_small: num=100, size=10, size.var=5
##   aim-haf_compare_d50_medium: num=50, size=25, size.var=12.5

num <- 100 # number of plants to simulate
size <- 10 # average size of plants (in # of cells)
size.var <- 5 # variance in size of plants
num.iter <- 100 # number of iterataions
out.file <- "aim-haf_compare_d100_small.csv" #name of the output file
...
for (j in 1:num.iter) {
  ...
  # empty raster stack to hold the simulated plants
  vegHeights <- stack()
  ...
  print(paste("Average AIM Height: ",aim.sum,sep=""))
  print(paste("Number of AIM height measurements: ",aim.count,sep=""))
  
  ## Now do the HAF height method.
  ...
  # filter out duplicate buffer IDs
  ...
  print(paste("Average HAF Height: ",haf.sum,sep=""))
  print(paste("Number of HAF height measurements: ",haf.count,sep=""))
  ...
  print(paste("Actual Average Plant Height: ",plot.avg,sep=""))
  print(paste("Actual Average Maximum Height: ",plot.max,sep=""))
...
## Clean up the output data frame and write it out to a CSV file
